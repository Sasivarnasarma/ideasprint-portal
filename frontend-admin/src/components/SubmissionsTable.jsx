  ...
  const filteredAndSortedSubmissions = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (team) => team.name.toLowerCase().includes(lowerQuery) || team.id.toString().toLowerCase().includes(lowerQuery),
      );
    }
            ...
            {filteredAndSortedSubmissions.map((team) => (
              ...
              >
                ...
                <td data-label="Team Name" style={{ padding: '1rem', fontWeight: 'bold' }}>
                  {team.name}{' '}
                  <span
                    style={{ color: 'var(--color-accent)', fontSize: '0.85rem', marginLeft: '0.5rem', opacity: 0.8 }}
                  >
                    ({team.level.replace('Level ', 'L')})
                  </span>
                </td>
