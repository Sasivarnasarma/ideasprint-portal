...
const RegisterStage3 = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await registrationAPI.register(finalData);
        ...
        {level === 'Level 1' && (
          <>
            ...
            >
              ...
              <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.6)', fontWeight: 'normal' }}>
                Max 100 words
              </span>
            </div>
            <div className="form-group" style={{ marginBottom: '0.5rem' }}>
              ...
              <div
                style={{
                  ...
                  fontSize: '0.8rem',
                  marginTop: '0.25rem',
                  color: idea.trim().split(/\s+/).filter(Boolean).length > 100 ? '#ff6b6b' : 'rgba(255,255,255,0.5)',
                }}
              >
          ...
          {members.map((member, index) => (
            <div key={index} className="member-card">
              <div className="member-header">
                ...
                {members.length > 2 && (
                  <button type="button" className="remove-member-btn" onClick={() => removeMember(index)}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="member-fields">
                <div className="member-form-group">
                  ...
                  />
                  {fieldErrors[`member-${index}-name`] && (
                    <span className="field-error">{fieldErrors[`member-${index}-name`]}</span>
                  )}
                </div>
                <div className="member-form-group">
                  <input
                    ...
                    required={index < 2}
                  />
                  {fieldErrors[`member-${index}-phone`] && (
                    <span className="field-error">{fieldErrors[`member-${index}-phone`]}</span>
                  )}
                </div>
                <div className="member-form-group">
                  <input
                    ...
                    required={index < 2}
                  />
                  {fieldErrors[`member-${index}-im_number`] && (
                    <span className="field-error">{fieldErrors[`member-${index}-im_number`]}</span>
                  )}
                </div>
              </div>
            </div>
