...
const RegisterStage2 = ({ onNext, onBack }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await authAPI.verifyOTP(emailToVerify, otpCode, registrationData.captchaSessionToken);
    ...
    const handleResend = async () => {
        ...
        try {
            const emailToVerify = registrationData.pendingEmail || registrationData.email;
            await authAPI.resendOTP(emailToVerify, registrationData.captchaSessionToken);
            setResendTimer(60);
            setError('');
        } catch (err) {
            if (err.response?.status === 401) {
