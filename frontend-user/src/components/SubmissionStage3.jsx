    ...
    return (
        <div className="stage-content">
            <div className="stage-header" style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ marginTop: '0.5rem', background: 'rgba(3, 199, 179, 0.1)', padding: '0.5rem 1rem', borderRadius: '8px', display: 'inline-block' }}>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(251,255,254,0.7)', textTransform: 'uppercase', letterSpacing: '1px' }}>Team</p>
                    <p style={{ margin: 0, fontSize: '1.1rem', fontWeight: 'bold', color: '#03C7B3' }}>{submissionData.teamName}</p>
                </div>
            </div>

            {submissionData.hasSubmitted && (
                <div style={{ background: 'rgba(255, 193, 7, 0.1)', borderLeft: '4px solid #ffc107', padding: '1rem', marginBottom: '1.5rem', borderRadius: '4px' }}>
                    <p style={{ margin: 0, color: '#ffb300', fontSize: '0.95rem' }}>
                        <strong>Warning:</strong> Your team has already submitted a proposal. Submitting a new one will overwrite your previous submission!
                    </p>
                </div>
            )}

            <form onSubmit={handleSubmit} className="auth-form" style={{ marginTop: '0', display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="form-group" style={{ marginBottom: '1.25rem' }}>
                    ...
                    >
                        ...
                        {formData.proposalPdf && !fieldErrors.proposalPdf ? (
                            <div className="selected-file" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                            ...
                            <div className="upload-prompt" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(3, 199, 179, 0.6)" strokeWidth="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                                    <polyline points="17 8 12 3 7 8" />
                                    <line x1="12" y1="3" x2="12" y2="15" />
                                </svg>
                                ...
                                <p style={{ margin: 0, color: 'rgba(251,255,254,0.5)', fontSize: '0.85rem' }}>
                                    or click to browse
                                </p>
                            </div>
                        )}
                ...
                {isLoading && uploadProgress > 0 && (
                    <div className="upload-progress" style={{ marginBottom: '1rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', fontSize: '0.85rem', color: 'rgba(251,255,254,0.8)' }}>
                            <span>Uploading...</span>
                            <span>{uploadProgress}%</span>
                        </div>
                        <div style={{ width: '100%', height: '6px', background: 'rgba(251,255,254,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{ width: `${uploadProgress}%`, height: '100%', background: '#03C7B3', transition: 'width 0.2s ease' }}></div>
                        </div>
                    </div>
                )}
